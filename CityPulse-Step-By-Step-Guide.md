...
3. **(Optional but recommended)**: Add a **Manage fields** transformation node. Keep *only* the following 5 columns and change their Data Types:
   - `DOLocationID` -> `long` (or `int`)
   - `tpep_pickup_datetime` -> `datetime`
   - `tpep_dropoff_datetime` -> `datetime`
   - `trip_distance` -> `float` (or `double`)
   - `total_amount` -> `float` (or `double`)
   > *Why we do this: Dropping unnecessary columns saves storage, and converting data types at the very edge of the pipeline ensures our Bronze, Silver, and Gold layers inherit the perfect schema automatically.*
4. Click **New destination** > **Eventhouse**. Select `eh_citypulse` and create a new table: `TaxiTrips`.
5. **Publish** both Eventstreams.
...
8. Use T-SQL to build a Gold layer star schema using Cross-Database querying. 
   ...
   **Fact Tables**:
   ...
   SELECT 
       ...
       CAST(tpep_pickup_datetime AS DATETIME2) AS PickupTime,
       CAST(tpep_dropoff_datetime AS DATETIME2) AS DropoffTime,
...
6. **Build Page 1 (Historical/Context)**:
   - **Line Chart**: 
     - *Title*: `Average Taxi Fare Over Time`
     - *X-Axis*: `PickupTime` (from `fact_taxi_trips`)
     - *Y-Axis*: `Avg Taxi Fare`
   - **Bar Chart**: 
     - *Title*: `Taxi Trips by Dropoff Borough`
     - *X-Axis*: `Borough` (from `dim_taxi_zone`)
     - *Y-Axis*: `Total Taxi Trips`
   - **Map Visual**: 
     - *Title*: `Average Bike Availability by Station`
     - *Latitude*: `Latitude` (from `dim_bike_station`) -> **Set to "Don't summarize"**
     - *Longitude*: `Longitude` (from `dim_bike_station`) -> **Set to "Don't summarize"**
     - *Bubble Size*: `Avg Bike Availability`
...
4. Create the following KQL-backed visuals for your live operations center:
   - **Visual 1 (Stat / Gauge)**: 
     - *Title*: `Total Taxi Trips Ingested (Live)`
     - *KQL Query*: `TaxiTrips | count`
     > *Note: The Fabric taxi sample stream replays data from 2022. Using a raw `count` allows you to watch the total number of trips climb rapidly in real-time as the stream runs, rather than returning 0 due to old timestamps.*
